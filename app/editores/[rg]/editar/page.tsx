...
export default function EditarEditorPage({ params }: { params: { rg: string } }) {
  ...
  useEffect(() => {
    const loadEditor = async () => {
      try {
        setIsLoading(true)
        const data = await fetchData<Editor>(`/editores/${rg}`)
        setEditor(data)
        setDtContrato(data.dtContrato ? new Date(data.dtContrato) : undefined)
      } catch (error) {
        toast({
          title: "Erro ao carregar editor",
          description: error instanceof Error ? error.message : "Ocorreu um erro ao carregar os dados do editor.",
          variant: "destructive",
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "O editor foi atualizado com sucesso.",
      })

      router.push("/editores")
    } catch (error) {
      toast({
        title: "Erro ao atualizar editor",
        description: error instanceof Error ? error.message : "Ocorreu um erro ao atualizar o editor.",
        variant: "destructive",
      })
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto max-w-2xl">
        <h1 className="text-3xl font-bold mb-6">Editar Editor</h1>
        <div className="flex justify-center items-center h-64">
          <p>Carregando dados do editor...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto max-w-2xl">
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            <CardTitle>Editar Editor</CardTitle>
            <CardDescription>Atualize as informações do editor</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="editorRg">RG do Editor</Label>
              <Input id="editorRg" value={rg} disabled />
            ...
            <div className="space-y-2">
              ...
              <Input id="nomeEmpregado" value={editor.nomeEmpregado} disabled />
